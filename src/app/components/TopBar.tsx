...
const TopBar = () => {
    ...
    useGSAP(() => {
        if (navbardesktop.current) {
            gsap.from(navbardesktop.current, {
                y: -40,
                opacity: 0,
                duration: 0.5,
                ease: "power2.out",
    ...
    useEffect(() => {
        if (mobileMenu && navbarmobile.current) {
            gsap.from(navbarmobile.current.children, {
                opacity: 0,
                stagger: 0.2
            })
        }
    ...
    return (
        <>
                ...
                <ul id="navlistitems" className='w-full flex justify-evenly items-center gap-3 '>
                    ...
                    <li>
                        <div className={`cursor-pointer w-[54px] h-[32px] bg-white rounded-3xl inline-flex items-center p-1 ${darkTheme ? 'bg-black' : 'bg-white'}`} onClick={() => setdarkTheme(!darkTheme)}>
                            ...
                            <span className={`w-1/2 h-full rounded-full grid place-content-center transition-transform duration-300 ease-in-out ${darkTheme ? "bg-[#2f3792] rotate-[260deg]" : ""}`}>
            ...
            <div className="h-full w-full flex justify-between items-center bg-[#333333] px-4 lg:hidden">
                ...
                {
                    mobileMenu && <ul ref={navbarmobile} className={`fixed h-full overflow-hidden inset-0 flex flex-col justify-start bg-[inherit] items-center gap-5 pt-32`}>
